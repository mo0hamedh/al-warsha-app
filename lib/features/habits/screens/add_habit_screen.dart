...
class _AddHabitScreenState extends State<AddHabitScreen> {
  ...
  Widget _buildStep1TypeSelection(ThemeProvider theme) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('اختر نوع العادة', style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 24, fontWeight: FontWeight.bold)),
          ...
          children: [
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(title, style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 20, fontWeight: FontWeight.bold)),
                  Text(subtitle, style: GoogleFonts.tajawal(color: theme.textSecondary, fontSize: 13)),
                ],
              ),
            )
          ],
    ...
    return Column(
      children: [
        Padding(
          ...
          child: Text('اختر العادة', style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 24, fontWeight: FontWeight.bold)),
        ),
        Expanded(
  ...
  Widget _buildStep3DurationSelection(ThemeProvider theme) {
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
           Text('ما هو هدفك؟', style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 24, fontWeight: FontWeight.bold)),
           const SizedBox(height: 8),
           Text('اختر مدة التحدي لتعويد عقلك', style: GoogleFonts.tajawal(color: theme.textSecondary, fontSize: 16)),
           const SizedBox(height: 40),
           ...
           ElevatedButton(
             ...
             child: Text('التالي', style: GoogleFonts.tajawal(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
           )
        ],
      ),
    );
  ...
  Widget _durationTile(int days, String title, String subtitle, ThemeProvider theme) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? theme.accentColor.withValues(alpha: 0.1) : theme.card,
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: isSelected ? theme.accentColor : (theme.isDarkMode ? Colors.white12 : Colors.black12), width: isSelected ? 2 : 1),
        ),
        child: Row(
          textDirection: TextDirection.rtl,
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                   Text(title, style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 18, fontWeight: FontWeight.bold)),
                   Text(subtitle, style: GoogleFonts.tajawal(color: theme.textSecondary, fontSize: 13)),
                ],
              ),
            ),
            if (isSelected) Icon(Icons.check_circle, color: theme.accentColor, size: 28)
          ...
          children: [
            Expanded(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                   Text('مدة مخصصة', style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 18, fontWeight: FontWeight.bold)),
                   if (isSelected)
                     Padding(
                       ...
                       child: TextField(
                         ...
                         decoration: InputDecoration(
                           ...
                           isDense: true,
                           border: UnderlineInputBorder(borderSide: BorderSide(color: theme.accentColor)),
                         ),
                         onChanged: (val) {
                           setState(() {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 24),
          Text(typeText, style: GoogleFonts.tajawal(color: theme.textSecondary, fontSize: 18)),
          Text(_selectedHabit!.name, style: GoogleFonts.tajawal(color: theme.primaryText, fontSize: 32, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          Container(
             padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
             decoration: BoxDecoration(color: theme.card, borderRadius: BorderRadius.circular(8), border: Border.all(color: color)),
             child: Text('الهدف: $_selectedTargetDays يوم', style: GoogleFonts.tajawal(color: color, fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          ...
          ElevatedButton(
            ...
            child: Text('ابدأ التحدي 🚀', style: GoogleFonts.tajawal(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.white)),
          ),
