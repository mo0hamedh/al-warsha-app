    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('حدث خطأ أثناء الحفظ', style: GoogleFonts.tajawal()),
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: AlertDialog(
        ...
        title: Text('تصنيف جديد ✨', style: GoogleFonts.tajawal(color: theme.primaryText, fontWeight: FontWeight.bold)),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              Text('اختر اللون', style: GoogleFonts.tajawal(color: theme.textSecondary, fontWeight: FontWeight.bold)),
              ...
              const SizedBox(height: 24),
              Text('اختر أيقونة', style: GoogleFonts.tajawal(color: theme.textSecondary, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 12,
                runSpacing: 12,
                children: _icons.map((icon) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: isSelected ? (_selectedColor ?? theme.accentColor).withOpacity(0.15) : Colors.transparent,
                        borderRadius: BorderRadius.circular(12),
                        border: isSelected ? Border.all(color: _selectedColor ?? theme.accentColor) : Border.all(color: Colors.transparent),
                      ),
                      child: Icon(
                        icon,
                        color: isSelected ? (_selectedColor ?? theme.accentColor) : theme.textSecondary,
                        size: 28,
                      ),
                    ),
                  );
